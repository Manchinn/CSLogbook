...
| 58 | 03-27 | Dead code audit (6 agents, 2 rounds verify): ลบ 10 ไฟล์ -650 lines, fix CRLF line endings (886 files), fix PDF iframe rewrite loop in production (INTERNAL_BACKEND_URL), วิเคราะห์ upload volume missing |
...
## Session 44 (claude, 2026-03-11) — Unified Request Pages Design Pattern

### บริบท

3 หน้า student request (exam-submit, thesis-defense, system-test) มี CSS ซ้ำกัน ~70% แต่แยกอยู่คนละ module, layout ไม่ consistent, มี duplicated logic (status class mapping, date formatting, timeline rendering, student table)

### สิ่งที่ทำ

**1. Shared CSS Module** — รวม ~40 classes จาก 3 modules เดิมเข้า `src/styles/requestPage.module.css`
- ใช้ CSS variables แทน hardcoded hex ทั้งหมด (tag tokens ใน globals.css)
- `.header` (basic) + `.headerWithBack` (flex space-between สำหรับ phase2 pages)
- `composes` pattern สำหรับ tag variants

**2. Shared Utilities**
- `toneStyles.ts` → `toneClassName()` แทน nested ternary ที่ซ้ำ 3 หน้า
- `formatDateTime.ts` → `formatDateTime()`, `formatDate()`, `getBangkokDateString()`

**3. Shared Sub-Components**
- `RequestTimeline.tsx` — Timeline rendering (exam-submit + system-test)
- `StudentTable.tsx` — Student data table 4 columns (exam-submit + thesis-defense)
- `Phase2GateWarning.tsx` — Phase2 gate blocking notice (thesis-defense + system-test)
- `RejectionNotice.tsx` — Rejection/returned notice (ทั้ง 3 หน้า)

**4. Unified Page Structure** — ทุกหน้าใช้ลำดับ section เดียวกัน:
Header → Stepper → Gate Warning → Rejection → Error → Status Card → Details → Timeline → Form → Student Table → Actions

### ไฟล์ที่เปลี่ยน

| ไฟล์ | รายละเอียด |
|------|-----------|
| `src/app/globals.css` | เพิ่ม tag semantic CSS variables (8 ตัว) |
| `src/styles/requestPage.module.css` | **NEW** shared CSS module (~40 classes) |
| `src/lib/utils/toneStyles.ts` | **NEW** tone → CSS class mapping |
| `src/lib/utils/formatDateTime.ts` | **NEW** shared date formatting |
| `src/components/common/RequestTimeline.tsx` | **NEW** timeline component |
| `src/components/common/StudentTable.tsx` | **NEW** student table component |
| `src/components/common/Phase2GateWarning.tsx` | **NEW** gate warning component |
| `src/components/common/RejectionNotice.tsx` | **NEW** rejection notice component |
| `src/app/(app)/project/phase1/exam-submit/page.tsx` | Refactor ใช้ shared module + components |
| `src/app/(app)/project/phase2/thesis-defense/ThesisDefenseRequestContent.tsx` | Refactor ใช้ shared module + components |
| `src/app/(app)/project/phase2/system-test/SystemTestRequestContent.tsx` | Refactor ใช้ shared module + components |
