...
> **หมายเหตุสำคัญ:** คำว่า **“เจ้าหน้าที่ภาค”** ในระบบ CSLogbook หมายถึงผู้ใช้ที่ล็อกอินด้วยบทบาท `teacher` แต่มี `teacherType = 'support'`. ฝั่ง Backend จะเก็บสถานะนี้ไว้ในตาราง `teachers` และฝั่ง Frontend จะใช้ค่าดังกล่าวเพื่อเปิดเมนูเดียวกับฝั่ง Admin (เช่น จัดการนักศึกษา/เอกสาร/รายงาน).
...
## 4. ขั้นตอนเมื่อเพิ่มบทบาทใหม่ในระบบ

เมื่อทีมพัฒนาต้องการเพิ่มบทบาทผู้ใช้ใหม่ (เช่น เพิ่ม `coordinator`) ให้ตรวจสอบและอัปเดตส่วนประกอบหลักดังต่อไปนี้เพื่อให้ RBAC ทำงานถูกต้องทั้งระบบ:

1. **ขยาย ENUM ฝั่ง Backend**
    - ปรับ `sequelize` model `User` (`backend/models/user.js`) เพื่อเพิ่มค่าของ `role` ใหม่
    - ตรวจสอบ `services/authService.js` (เมธอด `getRoleSpecificData` และ `generateToken`) ว่าคืนข้อมูลเสริมให้บทบาทใหม่เรียบร้อย
2. **อัปเดต Middleware ตรวจสิทธิ์**
    - เพิ่มบทบาทใหม่ลงใน `checkRole([...])` ของไฟล์ `backend/middleware/authMiddleware.js`
    - หากบทบาทใหม่มีประเภทหรือสิทธิ์ย่อย (เช่น teacherType) ให้เพิ่มตัวตรวจสอบเฉพาะ (`checkTeacherType`, `checkTeacherPosition`) หรือสร้าง middleware ใหม่ตามความจำเป็น
3. **กำหนดเมนูและเส้นทางใน Frontend**
    - ปรับ `frontend/src/components/layout/menuConfig.js` เพื่อกำหนดว่าบทบาทใหม่จะเห็นเมนูไหนบ้าง
    - ตรวจสอบ `AuthContext` และ context อื่น ๆ (เช่น `StudentEligibilityContext`) ว่าต้องเก็บข้อมูลเพิ่มเติมเพื่อตัดสินใจแสดงผล UI หรือไม่
4. **อัปเดตเอกสารและการทดสอบ**
    - แก้ไขไฟล์อ้างอิงบทบาท (รวมถึงเอกสารฉบับนี้) ให้สะท้อนโครงสร้างล่าสุด
    - เพิ่ม/อัปเดต test case เกี่ยวกับ role guard เช่นใน `Sidebar.role.test.js`

การทำตามขั้นตอนข้างต้นจะช่วยให้บทบาทใหม่ถูกบูรณาการเข้าระบบได้ครบวงจรและลดความเสี่ยงจากสิทธิ์ที่เปิดกว้างเกินความจำเป็น