...
import { getStepDefinition, detectWorkflowType, isThesisStep } from './project2Helpers';
...
const TimelineItems = ({ items = [], onAction }) => {
  ...
  const processedItems = items.map((item) => {
    ...
    // Detect workflow type and get step definition for Project 2
    const stepKey = item.stepKey || item.key;
    const workflowType = item.workflowType || detectWorkflowType(stepKey);
    const isThesis = isThesisStep(stepKey);
    const stepDef = stepKey ? getStepDefinition(stepKey, workflowType) : null;
    
    // สร้าง icon - ใช้ icon จาก Project 2 definition ถ้ามี
    let icon = getStatusIcon(status);
    
    if (stepDef && isThesis && stepDef.icon) {
      // ใช้ icon เฉพาะ Project 2 พร้อมปรับสีตามสถานะ
      icon = React.cloneElement(stepDef.icon, {
        style: { 
          ...stepDef.icon.props.style,
          color: status === 'completed' ? '#52c41a' : stepDef.color 
        }
      });
    ...
    // ใช้ label จาก Project 2 definition ถ้ามี
    const title = stepDef?.label || item.name || item.title || 'ไม่มีชื่อขั้นตอน';
        ...
        <>
          <div style={{ marginBottom: 8 }}>
            <Space align="start">
              ...
              {isThesis && <Tag color="purple">ปริญญานิพนธ์</Tag>}
              {item.document && <Tag color="geekblue">{item.document}</Tag>}
            </Space>
          </div>
