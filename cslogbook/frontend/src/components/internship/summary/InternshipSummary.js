...
const InternshipSummary = ({ studentId }) => {
    ...
    const fetchSummaryData = async () => {
      try {
        ...
        const response = await axios.get(
          `/api/students/${studentId}/internship/summary`
        );
        setSummaryData(response.data);

        // Fetch time records
        const timeResponse = await axios.get(
          `/api/students/${studentId}/internship/time-records`
        );
  ...
  const calculateStatistics = (records) => {
    ...
    const uniqueDates = new Set(
      records.map((record) => format(new Date(record.date), "yyyy-MM-dd"))
    );
    ...
    <div className="internship-container print-container">
      ...
      <Card className="summary-header-card">
        <Row gutter={[24, 24]}>
          <Col xs={24} sm={24} md={12}>
            <Title level={2}>Internship Summary</Title>
                  ...
                  <div>
                    ...
                    <div className="detail-value">
                      {summaryData.startDate
                        ? format(new Date(summaryData.startDate), "dd MMM yyyy")
                        : "Not started"}{" "}
                      -
                      {summaryData.endDate
                        ? format(new Date(summaryData.endDate), "dd MMM yyyy")
                        : "Not finished"}
                    </div>
                  </div>
                </div>
              ...
              <Col xs={24} md={12}>
                <div className="detail-item">
                  ...
                  <div>
                    ...
                    <div className="detail-value">
                      {summaryData.company?.location || "Not specified"}
                    </div>
                  </div>
                </div>
              </Col>
              ...
              <Col xs={24} md={12}>
                <div className="detail-item">
                  ...
                  <div>
                    ...
                    <div className="detail-value">
                      {summaryData.department || "Not specified"}
                    </div>
                  </div>
                </div>
              </Col>
              ...
              <Col xs={24} md={12}>
                <div className="detail-item">
                  ...
                  <div>
                    ...
                    <div className="detail-value">
                      {summaryData.position || "Not specified"}
                    </div>
                  </div>
                </div>
              </Col>
            ...
            <Row gutter={[24, 24]} className="stats-row">
              <Col xs={12} sm={12}>
                <Card className="status-card">
                  ...
                  />
                  <Progress
                    percent={statistics.completedPercentage}
                    status="active"
                  />
                </Card>
              </Col>
              ...
              <Col xs={12} sm={12}>
                <Card className="status-card">
                  ...
                  />
                  <Text type="secondary">
                    Avg {statistics.averageHoursPerDay} hrs/day
                  </Text>
                </Card>
              </Col>
                      ...
                      <Tag
                        color={
                          summaryData.status === "Completed"
                            ? "success"
                            : summaryData.status === "In Progress"
                            ? "processing"
                            : "default"
                        }
                      >
      ...
      <Tabs defaultActiveKey="1" className="tab-container">
        ...
        <TabPane
          tab={
            <span>
              <FileTextOutlined /> Learning Outcomes
            </span>
          }
          key="2"
        >
              ...
              <Card title="Learning Experience" className="learning-outcome">
                <Paragraph>
                  {summaryData.learningOutcome ||
                    "No learning outcomes have been recorded yet."}
                </Paragraph>
                  ...
                  items={[
                    {
                      ...
                      children: `First Day - ${
                        summaryData.startDate
                          ? format(
                              new Date(summaryData.startDate),
                              "dd MMM yyyy"
                            )
                          : "Not started"
                      }`,
                    },
                    {
                      ...
                      color: "blue",
                      children: `Mid-term Evaluation - ${
                        summaryData.midtermDate
                          ? format(
                              new Date(summaryData.midtermDate),
                              "dd MMM yyyy"
                            )
                          : "Not scheduled"
                      }`,
                    },
                    {
                      ...
                      color:
                        summaryData.status === "Completed" ? "green" : "gray",
