...
const DEFENSE_UI_META = {
  ...
  [DEFENSE_TYPE_THESIS]: {
    ...
    description:
      "ติดตามคำขอสอบปริญญานิพนธ์ที่ผ่านการอนุมัติจากอาจารย์ บันทึกการตรวจสอบ และส่งออกข้อมูลสำหรับการนัดสอบ",
  },
};
...
const StaffKP02Queue = ({ defenseType = DEFENSE_TYPE_PROJECT1 }) => {
  ...
  const uiMeta =
    DEFENSE_UI_META[defenseType] || DEFENSE_UI_META[DEFENSE_TYPE_PROJECT1];
  ...
  const formatDateOnly = useCallback((value) => {
    ...
    return dt.locale("th").format("DD/MM/BBBB");
  }, []);
  ...
  const handleVerify = useCallback(
    (record) => {
      if (!isStaff) {
        message.warning(
          "เฉพาะเจ้าหน้าที่ที่ได้รับสิทธิ์เท่านั้นที่สามารถบันทึกการตรวจสอบได้"
        );
        return;
      }
      let noteValue = "";
      Modal.confirm({
        ...
        content: (
          <Space direction="vertical" style={{ width: "100%" }}>
            <Text type="secondary">
              ใส่หมายเหตุถึงนักศึกษา (ไม่จำเป็นต้องกรอก)
            </Text>
            <Input.TextArea
              rows={3}
              onChange={(event) => {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const { blob, filename } = await projectService.exportProject1StaffQueue({
        ...params,
        defenseType,
      });
          ...
          return (
            <Space direction="vertical" size={4}>
              <Space size={6} wrap>
                ...
                {project.projectCode && (
                  <Tag color="geekblue">{project.projectCode}</Tag>
                )}
              </Space>
              <Text type="secondary">
                ปีการศึกษา {project.academicYear || "-"} / ภาคเรียน{" "}
                {project.semester || "-"}
              </Text>
            </Space>
          );
      ...
      },
      {
        ...
        render: (_value, record) => (
          <Space direction="vertical" size={2}>
            ...
            <Text>
              ตารางสอบ:{" "}
              {record.defenseScheduledAt
                ? formatDateTime(record.defenseScheduledAt)
                : "ติดตามผ่านปฏิทินภาควิชา"}
            </Text>
          </Space>
        ),
      },
  ...
  const expandedRowRender = useCallback(
    (record) => {
      ...
      const projectMetricsPhase1 =
        record.project?.meetingMetricsPhase1 ||
        record.project?.meetingMetrics ||
        null;
      ...
      const fallbackPhaseMetrics = isThesis
        ? projectMetricsPhase1
        : projectMetricsPhase2;
      const fallbackPhaseLabel = isThesis ? "Phase 1" : "Phase 2";
      const attachments = Array.isArray(record.formPayload?.additionalMaterials)
        ? record.formPayload.additionalMaterials
        : [];
      ...
      const projectNameForEvidence =
        record.project?.projectNameTh ||
        record.project?.projectNameEn ||
        record.project?.projectCode ||
        "";

      const systemTestSnapshot = isThesis
        ? record.formPayload?.systemTestSnapshot || null
        : null;
      const systemTestMetaBase = systemTestSnapshot
        ? SYSTEM_TEST_STATUS_META[systemTestSnapshot.status] ||
          SYSTEM_TEST_STATUS_META.default
        : SYSTEM_TEST_STATUS_META.default;
      const systemTestMeta =
        systemTestSnapshot?.status === "staff_approved" &&
            ...
            <Space direction="vertical" size={16} style={{ width: "100%" }}>
              <Card size="small" title="รายละเอียดเพิ่มเติม">
                <Space direction="vertical" size={6} style={{ width: "100%" }}>
                  <Text>
                    หมายเหตุเจ้าหน้าที่: {record.staffVerificationNote || "-"}
                  </Text>
                  <Text>
                    เจ้าหน้าที่ผู้ตรวจ:{" "}
                  ...
                  >
                    ...
                    {isThesis && intendedDefenseDate && (
                      <Text>
                        วันที่คาดว่าจะสอบ: {formatDateOnly(intendedDefenseDate)}
                      </Text>
                    )}
                    {additionalNotes && (
                      <Text type="secondary" style={{ whiteSpace: "pre-wrap" }}>
                        หมายเหตุจากคำขอ: {additionalNotes}
                      </Text>
                    )}
                  </Space>
          ...
          <Col xs={24} md={10}>
            <Space direction="vertical" size={16} style={{ width: "100%" }}>
              ...
              >
                {meetingMetrics ? (
                  ...
                  >
                    <Space direction="vertical" size={2}>
                      <Text>
                        อนุมัติทั้งหมด: {meetingMetrics.totalApprovedLogs || 0}
                      </Text>
                      <Text>
                        ต้องมีขั้นต่ำ:{" "}
                        {meetingMetrics.requiredApprovedLogs || 0}
                      </Text>
                      <Text>
                        อนุมัติครั้งล่าสุด:{" "}
                        {formatDateTime(meetingMetrics.lastApprovedLogAt)}
                      </Text>
                    </Space>
                  </Space>
                ) : (
                  <Alert
                    type="info"
                    message="ไม่พบข้อมูลบันทึกการพบ"
                    showIcon
                  />
                )}
              </Card>
              {isThesis && fallbackPhaseMetrics && (
                <Card
                  size="small"
                  title={`ภาพรวมบันทึกการพบ (${fallbackPhaseLabel})`}
                >
                  <Space direction="vertical" size={6}>
                    <Text>
                      อนุมัติทั้งหมด:{" "}
                      {fallbackPhaseMetrics.totalApprovedLogs || 0}
                    </Text>
                    <Text>
                      ต้องมีขั้นต่ำ:{" "}
                      {fallbackPhaseMetrics.requiredApprovedLogs || 0}
                    </Text>
                    <Text>
                      อนุมัติครั้งล่าสุด:{" "}
                      {formatDateTime(fallbackPhaseMetrics.lastApprovedLogAt)}
                    </Text>
                  </Space>
                </Card>
              )}
              {isThesis && (
                <Card size="small" title="คำขอทดสอบระบบ 30 วัน">
                  {systemTestSnapshot ? (
                    ...
                    >
                      <Space size={8} wrap>
                        <Tag color={systemTestMeta.color}>
                          {systemTestMeta.text}
                        </Tag>
                        <Text type="secondary">
                          {systemTestSnapshot.staffDecidedAt
                            ? `อนุมัติล่าสุด ${formatDateTime(
                                systemTestSnapshot.staffDecidedAt
                              )}`
                            : `ยื่นคำขอเมื่อ ${formatDateTime(
                                systemTestSnapshot.testStartDate
                              )}`}
                        </Text>
                      </Space>
                      <Descriptions bordered size="small" column={1}>
                        ...
                        <Descriptions.Item label="วันอนุมัติล่าสุด">
                          {systemTestSnapshot.staffDecidedAt
                            ? formatDateTime(systemTestSnapshot.staffDecidedAt)
                            : "-"}
                        </Descriptions.Item>
                          ...
                          <Descriptions.Item label="ไฟล์หลักฐาน">
                            <Space size={8} wrap>
                              <Button
                                ...
                                onClick={() =>
                                  handleOpenEvidence(
                                    systemTestSnapshot.evidence,
                                    projectNameForEvidence
                                  )
                                }
                              >
                                เปิดในระบบ
                    ...
                    <Alert
                      type="info"
                      showIcon
                      message="ยังไม่มีคำขอทดสอบระบบ 30 วัน"
                    />
                  )}
                </Card>
              )}
