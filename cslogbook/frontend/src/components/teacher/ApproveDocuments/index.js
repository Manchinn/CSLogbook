...
import { Card, Table, Tag, Button, Space, message, Row, Col, Input, DatePicker, Segmented, Modal, Radio, Form, Select } from 'antd';
...
export default function ApproveDocuments() {
  ...
  const [filters, setFilters] = useState({ q: '', status: 'all', dateRange: [], academicYear: 'all', semester: 'all', classYear: 'all' });
  const [approveModal, setApproveModal] = useState({ open: false, record: null });
  ...
  const filteredItems = useMemo(() => {
    ...
    const computeAcademic = (dt) => {
      ...
      // นิยามง่าย: ส.ค.-ธ.ค. => ภาค 1 ของปี BE ปัจจุบัน, ม.ค.-พ.ค. => ภาค 2 ของปี BE ก่อนหน้า, อื่นๆ ปักเป็นภาค 2 ของปีก่อนหน้า
      if (m >= 8 && m <= 12) return { yearBE: d.year() + 543, semester: 1 };
      if (m >= 1 && m <= 5) return { yearBE: d.year() + 543 - 1, semester: 2 };
      return { yearBE: d.year() + 543 - 1, semester: 2 };
    };
    ...
    return items.filter((r) => {
      ...
      const matchQ = !q || name.includes(q) || company.includes(q) || code.includes(q);

      // สถานะ
      const matchStatus = filters.status === 'all' ? true : r.status === filters.status;

      // ช่วงวันส่ง
      const created = r.createdAt ? dayjs(r.createdAt) : null;
      const matchDate = !start || !end ? true : (created && created.isBetween(start, end, 'day', '[]'));

      // ปีการศึกษา/ภาคเรียน (ยึดตาม startDate เป็นหลัก ถ้าไม่มีใช้ createdAt)
      const baseDate = r.startDate || r.createdAt || null;
      const { yearBE, semester } = computeAcademic(baseDate);
      const matchYear = filters.academicYear === 'all' || (yearBE && String(yearBE) === String(filters.academicYear));
      const matchSemester = filters.semester === 'all' || (semester && String(semester) === String(filters.semester));
      ...
      return matchQ && matchStatus && matchDate && matchYear && matchSemester && matchClass;
    });
  }, [items, filters]);

  // ตัวเลือกปีการศึกษา: รวบรวมจากรายการ
  const academicYearOptions = useMemo(() => {
    ...
    items.forEach((r) => {
      ...
      const yearBE = m >= 8 ? dayjs(d).year() + 543 : dayjs(d).year() + 543 - 1;
      years.add(yearBE);
    });
    return Array.from(years).filter(Boolean).sort((a, b) => b - a).map((y) => ({ label: String(y), value: String(y) }));
  }, [items]);
  ...
  const onSubmitApprove = async () => {
    try {
      ...
      await internshipApprovalService.approveCS05(approveModal.record.documentId, { letterType, comment });
  ...
  return (
    <Card title="อนุมัติเอกสาร คพ.05" extra={<Button onClick={fetchQueue}>รีเฟรช</Button>}>
      {/* แถบตัวกรองเพื่อความสะดวก */}
      <Row gutter={[12,12]} style={{ marginBottom: 12 }}>
        <Col xs={24} md={8}>
          <Input placeholder="ค้นหา ชื่อ/รหัส/บริษัท" value={filters.q} onChange={(e) => setFilters((f) => ({ ...f, q: e.target.value }))} allowClear />
        </Col>
        <Col xs={24} md={8}>
          <DatePicker.RangePicker style={{ width: '100%' }} value={filters.dateRange} onChange={(v) => setFilters((f) => ({ ...f, dateRange: v }))} format="DD/MM/YYYY" />
        </Col>
        <Col xs={24} md={8}>
          <Segmented options={[{label:'ทั้งหมด', value:'all'},{label:'รอตรวจสอบ', value:'pending'},{label:'อนุมัติแล้ว', value:'approved'},{label:'ปฏิเสธแล้ว', value:'rejected'}]} value={filters.status} onChange={(v) => setFilters((f) => ({ ...f, status: v }))} />
        </Col>
      </Row>
      <Row gutter={[12,12]} style={{ marginBottom: 12 }}>
        <Col xs={24} md={8}>
        ...
        <Col xs={24} md={8}>
          <Segmented
            options={[{label:'ทุกภาค', value:'all'},{label:'ภาค 1', value:'1'},{label:'ภาค 2', value:'2'}]}
        ...
        <Col xs={24} md={8}>
          <Segmented
            options={[{label:'ทุกชั้นปี', value:'all'},{label:'ปี 3', value:'3'},{label:'ปี 4', value:'4'}]}
      ...
      <Table rowKey="documentId" loading={loading} columns={columns} dataSource={filteredItems} pagination={{ pageSize: 10 }} />
      ...
      <Modal
        ...
        onCancel={() => { setApproveModal({ open: false, record: null }); form.resetFields(); }}
        onOk={onSubmitApprove}
        okText="ยืนยันอนุมัติ"
        cancelText="ยกเลิก"
      >
        <Form form={form} layout="vertical" initialValues={{ letterType: 'referral_letter' }}>
          <Form.Item name="letterType" label="ประเภทหนังสือ" rules={[{ required: true, message: 'กรุณาเลือกประเภทหนังสือ' }]}>
            <Radio.Group>
              <Radio.Button value="referral_letter">หนังสือส่งตัวนักศึกษา</Radio.Button>
              <Radio.Button value="request_letter">หนังสือขอความอนุเคราะห์</Radio.Button>
            </Radio.Group>
          </Form.Item>
