// filepath: c:\Users\chinn\CSLog\cslogbook\frontend\src.new\components\internship\evaluation\SupervisorEvaluation.js
import React, { useState, useEffect, useCallback } from 'react';
import { 
  Form, Input, Button, Radio, Card, Typography, Space, 
...
const SupervisorEvaluation = () => {
  ...
  const fetchEvaluationData = useCallback(async () => { // Wrapped in useCallback
    try {
      setLoading(true);
  ...
  }, [token]); // Added token to useCallback dependency array

  useEffect(() => {
    fetchEvaluationData();
  }, [fetchEvaluationData]); // Added fetchEvaluationData to useEffect dependency array
