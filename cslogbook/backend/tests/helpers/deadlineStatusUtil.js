// Utility แยกตรรกะคำนวณสถานะ deadline เพื่อให้ง่ายต่อ unit test
// NOTE: ตรรกะต้องสอดคล้อง importantDeadlineController.getAllForStudent
...
function computeStatus(deadline, submission, now = new Date()) {
  ...
  if (deadline.windowStartAt) {
    ...
    if (ws <= nowMs && nowMs <= effMs) {
      return { status: 'in_window', locked: false };
    }
