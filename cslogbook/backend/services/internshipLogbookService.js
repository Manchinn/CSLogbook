...
class InternshipLogbookService {
  ...
  async getTimeSheetEntries(userId) {
    try {
      logger.info(`InternshipLogbookService: ดึงข้อมูลบันทึกการฝึกงานสำหรับผู้ใช้ ${userId}`);
      ...
      logger.info(`InternshipLogbookService: พบบันทึกการฝึกงาน ${entries.length} รายการ`);
      return entries;

    } catch (error) {
      logger.error('InternshipLogbookService: Error in getTimeSheetEntries', error);
      throw error;
    }
  }
  ...
  async saveTimeSheetEntry(userId, entryData) {
    ...
    try {
      logger.info(`InternshipLogbookService: บันทึกข้อมูลการฝึกงานสำหรับผู้ใช้ ${userId}`);
      ...
      logger.info(`InternshipLogbookService: บันทึกข้อมูลการฝึกงานสำเร็จ ID: ${entry.logId}`);
      return entry;

    } catch (error) {
      await transaction.rollback();
      logger.error('InternshipLogbookService: Error in saveTimeSheetEntry', error);
      throw error;
    }
  }
  ...
  async updateTimeSheetEntry(userId, logId, updateData) {
    ...
    try {
      logger.info(`InternshipLogbookService: อัปเดตบันทึกการฝึกงาน ID: ${logId}`);
      ...
      await transaction.commit();
      logger.info(`InternshipLogbookService: อัปเดตบันทึกการฝึกงานสำเร็จ ID: ${logId}`);
      return entry;

    } catch (error) {
      await transaction.rollback();
      logger.error('InternshipLogbookService: Error in updateTimeSheetEntry', error);
      throw error;
    }
  }
  ...
  async getTimeSheetStats(userId) {
    try {
      logger.info(`InternshipLogbookService: ดึงสถิติการฝึกงานสำหรับผู้ใช้ ${userId}`);
      ...
      if (!document) {
        throw new Error('ไม่พบข้อมูล CS05 ที่รออนุมัติหรือได้รับการอนุมัติแล้ว');
      }
      ...
      logger.error('InternshipLogbookService: Error in getTimeSheetStats', error);
      throw error;
    }
  }
  ...
  async getInternshipDateRange(userId) {
    try {
      logger.info(`InternshipLogbookService: ดึงช่วงวันที่ฝึกงานสำหรับผู้ใช้ ${userId}`);
      ...
      logger.error('InternshipLogbookService: Error in getInternshipDateRange', error);
      throw error;
    }
  }
  ...
  async generateInternshipDates(userId) {
    try {
      logger.info(`InternshipLogbookService: สร้างรายการวันทำงานสำหรับผู้ใช้ ${userId}`);
      ...
      logger.info(`InternshipLogbookService: สร้างรายการวันทำงานสำเร็จ ${workdays.length} วัน`);
      return workdays;

    } catch (error) {
      logger.error('InternshipLogbookService: Error in generateInternshipDates', error);
      throw error;
    }
  }
  ...
  async getTimeSheetEntryById(userId, logId) {
    try {
      logger.info(`InternshipLogbookService: ดึงบันทึกการฝึกงาน ID: ${logId} สำหรับผู้ใช้ ${userId}`);
      ...
      logger.info(`InternshipLogbookService: ดึงบันทึกการฝึกงานสำเร็จ ID: ${logId}`);
      return entry;

    } catch (error) {
      logger.error('InternshipLogbookService: Error in getTimeSheetEntryById', error);
      throw error;
    }
  }
  ...
  async checkIn(userId, checkInData) {
    ...
    try {
      logger.info(`InternshipLogbookService: บันทึกเวลาเข้างานสำหรับผู้ใช้ ${userId}`);
  ...
  async checkOut(userId, checkOutData) {
    ...
    try {
      logger.info(`InternshipLogbookService: บันทึกเวลาออกงานสำหรับผู้ใช้ ${userId}`);
      ...
      if (!entry) {
        throw new Error('ไม่พบข้อมูลการบันทึกเวลาเข้างาน กรุณาบันทึกเวลาเข้างานก่อน');
      }
  ...
  async approveTimeSheetEntry(teacherId, logId, comment) {
    ...
    try {
      logger.info(`InternshipLogbookService: อนุมัติบันทึกการฝึกงาน ID: ${logId} โดยอาจารย์ ${teacherId}`);
      ...
      if (!entry) {
        throw new Error('ไม่พบข้อมูลบันทึกการฝึกงานหรือคุณไม่ใช่อาจารย์ที่ปรึกษาของนักศึกษาคนนี้');
      }
      ...
      await transaction.commit();
      logger.info(`InternshipLogbookService: อนุมัติบันทึกการฝึกงานสำเร็จ ID: ${logId}`);
      return entry;

    } catch (error) {
      await transaction.rollback();
      logger.error('InternshipLogbookService: Error in approveTimeSheetEntry', error);
      throw error;
    }
  }
  ...
  async saveReflection(userId, reflectionData) {
    ...
    try {
      logger.info(`InternshipLogbookService: บันทึกบทสรุปการฝึกงานสำหรับผู้ใช้ ${userId}`);
      ...
      const { learningOutcome, keyLearnings, futureApplication, improvements } = reflectionData;
  ...
  async getReflection(userId) {
    try {
      logger.info(`InternshipLogbookService: ดึงบทสรุปการฝึกงานสำหรับผู้ใช้ ${userId}`);
