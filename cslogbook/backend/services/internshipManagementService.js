...
class InternshipManagementService {
   ...
   */
  async submitCS05(userId, { companyName, companyAddress, startDate, endDate }) {
   ...
   */
  async submitCompanyInfo(documentId, userId, { supervisorName, supervisorPhone, supervisorEmail }) {
    const transaction = await sequelize.transaction();
    try {
      const document = await Document.findOne({
        where: { 
  ...
  async getEvaluationStatus(userId) {
    ...
    // ค้นหา InternshipDocument ผ่าน Document
    ...
    const evaluationRequest = await ApprovalToken.findOne({
      where: {
        requestType: 'supervisor_evaluation',
        requestData: {
    ...
    if (evaluationRequest) {
      status = evaluationRequest.status === 'completed' ? 'completed' : 'sent';
   ...
   */
  async sendEvaluationForm(internshipId, userId) {
    const transaction = await sequelize.transaction();
    try {
      ...
      if (!document || !document.internshipDocument) {
        throw new Error('ไม่พบเอกสาร CS05 ที่ได้รับการอนุมัติ หรือข้อมูลการฝึกงานไม่สมบูรณ์');
      }
      ...
      if (existingToken) {
        throw new Error(`คำขอประเมินผลถูกส่งไปยัง ${document.internshipDocument.supervisorEmail} แล้ว และยังไม่หมดอายุ (หมดอายุ ${existingToken.expiresAt.toLocaleDateString('th-TH')})`);
      }
      ...
      const supervisorName = document.internshipDocument.supervisorName || 'ผู้ควบคุมการฝึกงาน';
  ...
  async getSupervisorEvaluationFormDetails(token) {
    ...
    if (!internshipDetails || !studentUser || !studentRecord) {
      throw new Error('ไม่พบข้อมูลการฝึกงานหรือข้อมูลนักศึกษาที่เกี่ยวข้องอย่างสมบูรณ์');
    }
  ...
  async submitSupervisorEvaluation(token, evaluationData) {
    ...
    try {
      ...
      if (!approvalToken) {
        const error = new Error('แบบฟอร์มการประเมินไม่ถูกต้อง, หมดอายุ หรือถูกใช้งานไปแล้ว');
      ...
      const evaluation = await InternshipEvaluation.create({
        ...
        evaluatorName: evaluationData.supervisorName || internshipDocument.supervisorName,
