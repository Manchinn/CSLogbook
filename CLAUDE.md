...
ดู `.env.example` สำหรับรายละเอียด ตัวแปรสำคัญ: `NODE_ENV`, `DB_*`, `JWT_SECRET` (≥32 chars), `EMAIL_PROVIDER`, `ENABLE_AGENTS`
...
ดู `env.example` — ต้องมี `NEXT_PUBLIC_API_URL` (required at build time)
...
Location: `/home/deploy-cslogbook/app/cslogbook/.env.production` — ต้องเพิ่ม `NODE_ENV=production`, `BASE_URL`, `FRONTEND_URL`, `NEXT_PUBLIC_API_URL`, `MYSQL_ROOT_PASSWORD`
...
ดูรายละเอียดแต่ละ session ได้ที่ `docs/SESSION_LOG.md`

### สรุปสิ่งที่ทำแล้ว (Sessions 1-22)

- Bug fixes: login error, status labels, certificate PDF Thai font, query key mismatch
- Security: RBAC บน admin endpoints, report routes, dead route cleanup
- UX/UI: ConfirmDialog, Skeleton loading, DefenseRequestStepper, SurveyBanner
- Features: Unified project page (Phase1+2), teacher queue filters + summary stats, timesheet delete, referral letter, 404 page
- Infra: CI/CD pipeline, CORS config, Docker healthcheck
- Naming: "โครงงานพิเศษ 2" → "ปริญญานิพนธ์" ทั่วระบบ

### งานที่ยังต้องทำ

- Staging regression testing ตาม `docs/STAGING_TEST_PLAN.md`
- Student result pages (out of scope ตั้งใจ)
